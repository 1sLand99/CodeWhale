...
Codewhale は、選んだモデルを使ってソフトウェアの開発、ファイルの操作、日常的な作業の自動化を支援するオープンソースのエージェントです。ターミナルでタスクを指定し、ホスト型またはローカルのモデルに接続して始められます。大きな仕事を異なるモデルや役割に分担させたいときは、エージェントのチームも利用できます。
