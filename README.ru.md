...
Чего нет у других харнессов: **модель для каждой роли выбираете вы — и они
не обязаны совпадать** — и **агенты в Codewhale общаются друг с другом,
через модели.** Fleet закрепляет провайдера, модель и уровень рассуждений за
каждой ролью, поэтому дешёвая быстрая модель может руководить дорогой
рассуждающей, а builder на GLM может делать ту же задачу, что и reviewer на
Kimi. Пока они работают, можно отправить любому из них записку прямо в
полёте, заглянуть в его transcript или прервать его — и это не только
«родитель–ребёнок»: отдельные задачи Codewhale в одном workspace
обмениваются прочным Agent Mail, который переживает перезапуск, доставляется
ровно один раз на безопасной границе и маскирует учётные данные. `/goal`
держит долгую цель через много ходов, пока она действительно не завершена.
Роли — это файлы, которые вы редактируете, и весь харнесс остаётся вашим.
...
- **Любая модель, любой провайдер.** DeepSeek, Claude, GPT, Kimi, GLM и более 30
  ...
  а не как $0.
- **Агенты, которые общаются между собой — через модели.** Любой агент в
  Codewhale доступен, пока работает: `message` ставит записку в очередь
  работающему сабагенту, `followup` будит его с вашей запиской на следующей
  безопасной границе, `peek` читает его transcript, а прерывание
  останавливает только его ход. Это выходит за рамки дерева
  «родитель–ребёнок»: отдельные задачи в одном workspace обмениваются
  прочным **Agent Mail** — поставленное в очередь резюме-передача, которое
  переживает перезапуск, доставляется ровно один раз на безопасной границе
  получателя и маскирует учётные данные и пути. Так сессия на GLM и сессия
  на Kimi координируются в двух терминалах, а вы выпадаете из роли
  ретранслятора. Стороны могут быть на разных моделях; Codewhale доносит
  разговор.
...
- **Работа, которую можно продолжить.** Fleet записывает каждый шаг в
  append-only журнал, и `fleet resume` продолжает с места остановки. `/goal`
  держит устойчивую цель, к которой агент идёт ход за ходом — её можно
  поставить на паузу, возобновить, и она восстанавливается вместе с сессией
  после перезапуска, — а `/workflows` открывает живую панель по всем
  запускам, которые хранит журнал этого workspace.
