...
export function UsageConsent({ locale, appVersion }: { locale: string; appVersion: string }) {
  ...
  // The open sheet takes focus and gives it back; Escape closes it only once
  // a decision exists (an undecided sheet is dismissed by deciding).
  useEffect(() => {
    if (!open) return;
    const sheet = sheetRef.current;
    sheet?.querySelector<HTMLElement>("button")?.focus();
    const onKey = (event: KeyboardEvent) => {
      if (event.key === "Escape" && state !== "undecided") {
  ...
  return (
    ...
    >
      <div className="usage-consent-copy">
        <strong>{t(USAGE_CONSENT_COPY.title)}</strong>
        <p>{t(USAGE_CONSENT_COPY.body)}</p>
      ...
      <div className="usage-consent-actions">
        {undecided ? (
          ...
          <>
            ...
            <button type="button" className="usage-consent-button" onClick={() => setOpen(false)}>
              {t(USAGE_CONSENT_COPY.close)}
            </button>
          </>
        )}
      </div>
