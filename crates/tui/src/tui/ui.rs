...
/// Stream events a local cancel hides until the turn completes. Approval and
/// question requests are never silently hidden: `resolve_stale_parent_request`
/// answers a stale parent request explicitly, and a child agent's request is
/// always delivered (approvals C1).
