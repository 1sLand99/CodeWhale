...
pub trait ModalView: std::any::Any {
    ...
    /// The Fleet launch governor's one-line state (addendum F5), or `None`
    /// at full concurrency. Only the `/subagents` register shows it.
    fn update_launch_throttle(&mut self, _line: Option<&str>) -> bool {
        false
    }
...
pub struct SubAgentsView {
    ...
    /// The launch governor's line (addendum F5): why new agents wait while
    /// provider rate limits hold launches below the configured concurrency.
    /// `None` at full concurrency, so a calm Fleet shows nothing extra.
    launch_throttle: Option<String>,
...
impl ModalView for SubAgentsView {
    ...
    fn update_launch_throttle(&mut self, line: Option<&str>) -> bool {
        if self.launch_throttle.as_deref() == line {
            return false;
        }
        self.launch_throttle = line.map(str::to_string);
        true
    }
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        Paragraph::new(vec![
            ...
            // F5: the governor line takes the spacer row only while launches
            // are held back, so a Fleet at full concurrency stays quiet.
            match self.launch_throttle.as_deref() {
                Some(line) => Line::from(Span::styled(
                    format!(
                        "  {}",
                        truncate_view_text(line, content_width.saturating_sub(2))
                    ),
...
mod tests {
    ...
    #[test]
    fn register_header_names_the_launch_governor_only_while_it_holds_launches() {
        let area = Rect::new(0, 0, 110, 20);
        let mut view = SubAgentsView::new(vec![manager_agent("a", SubAgentStatus::Running)]);
        let line = "launch slots throttled to 2/8 after 3 provider rate limit(s) in the last 60s";

        let mut calm = Buffer::empty(area);
        view.render(area, &mut calm);
        assert!(!buffer_text(&calm, area).contains("throttled"));

        assert!(view.update_launch_throttle(Some(line)));
        assert!(
            !view.update_launch_throttle(Some(line)),
            "unchanged line is not an update"
        );
        let mut throttled = Buffer::empty(area);
        view.render(area, &mut throttled);
        assert!(buffer_text(&throttled, area).contains(line));

        assert!(view.update_launch_throttle(None));
        let mut recovered = Buffer::empty(area);
        view.render(area, &mut recovered);
        assert!(!buffer_text(&recovered, area).contains("throttled"));
    }
