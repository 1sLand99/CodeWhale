...
mod tests {
    ...
    fn snapshot() -> ExtensionsSnapshot {
        ...
        snapshot.tabs[ExtensionsTab::Plugins.index()] = ExtensionsTabModel {
            groups: vec![
                ExtensionGroup {
                    ...
                    items: vec![
                        item("browser", "Browser Pack", "controls a browser"),
                        item("sandbox", "Sandbox Pack", "isolates commands"),
                    ],
                },
                ExtensionGroup {
                    id: "installed".into(),
                    label: "Installed".into(),
                    items: vec![item("git", "Git tools", "repository helpers")],
                },
            ],
            problem: None,
        };
    ...
    fn render_text(view: &ExtensionsView, width: u16, height: u16) -> String {
        let backend = TestBackend::new(width, height);
        let mut terminal = Terminal::new(backend).unwrap();
        terminal
            .draw(|frame| frame.render_widget(ViewWidget(view), frame.area()))
            .unwrap();
        terminal
            .backend()
            .buffer()
    ...
    #[test]
    fn search_reveals_folded_groups_without_mutating_fold_state() {
        let mut view = ExtensionsView::from_snapshot(snapshot(), ExtensionsTab::Plugins);
        view.folded_groups.insert("1:recommended".into());
        view.query = "browser".into();

        let visible = view.visible_entries();

        assert_eq!(visible.len(), 2);
        assert!(matches!(visible[0], VisibleEntry::Group(_)));
        assert!(matches!(visible[1], VisibleEntry::Item(_, _)));
        assert_eq!(view.folded_group_count(), 1);
        view.query.clear();
    ...
    #[test]
    fn group_headers_are_keyboard_selectable_and_foldable() {
        let mut view = ExtensionsView::from_snapshot(snapshot(), ExtensionsTab::Plugins);
        assert!(matches!(view.visible_entries()[0], VisibleEntry::Group(_)));
    ...
    #[test]
    fn tab_and_shift_tab_cycle_focus_zones() {
        let mut view = ExtensionsView::from_snapshot(snapshot(), ExtensionsTab::Plugins);
        assert_eq!(view.focus, ExtensionsFocus::List);
        view.handle_key(KeyEvent::new(KeyCode::Tab, KeyModifiers::NONE));
        assert_eq!(view.focus, ExtensionsFocus::Tabs);
        view.handle_key(KeyEvent::new(KeyCode::Tab, KeyModifiers::SHIFT));
        assert_eq!(view.focus, ExtensionsFocus::List);
    }

    #[test]
    fn search_filters_only_the_active_tab() {
        let mut snapshot = snapshot();
        snapshot.tabs[ExtensionsTab::Hooks.index()] = ExtensionsTabModel {
            groups: vec![ExtensionGroup {
                id: "hooks".into(),
                label: "Hooks".into(),
                items: vec![item("browser-hook", "Browser hook", "browser")],
            }],
            problem: None,
        };
        let mut view = ExtensionsView::from_snapshot(snapshot, ExtensionsTab::Plugins);
        ...
        view.set_tab(ExtensionsTab::Hooks);
        assert_eq!(view.visible_entries().len(), 2);
    }

    #[test]
    fn compact_40_by_12_keeps_tabs_status_focus_and_footer_but_drops_description() {
        let view = ExtensionsView::from_snapshot(snapshot(), ExtensionsTab::Plugins);
    ...
    #[test]
    fn spacious_layout_includes_descriptions() {
        let view = ExtensionsView::from_snapshot(snapshot(), ExtensionsTab::Plugins);
        let text = render_text(&view, 100, 24);
        assert!(text.contains("controls a browser"), "{text}");
    }

    #[test]
    fn plugin_product_is_one_row_with_real_components() {
        let product = PluginProduct {
            id: "browser-pack".into(),
            name: "Browser Pack".into(),
            description: "Browser automation".into(),
            publisher: "Example".into(),
            source_reference: "example/browser-pack@abc123".into(),
            components: vec![
                PluginProductComponent {
                    kind: PluginProductComponentKind::Mcp,
            ...
            maturity: "reviewed".into(),
        };
        let snapshot =
            ExtensionsSnapshot::default().with_recommendations(vec![product], Locale::En);
        let rows = &snapshot.tab(ExtensionsTab::Marketplace).groups[0].items;
        assert_eq!(rows.len(), 1);
        assert!(rows[0].detail.contains("Playwright (MCP)"));
        assert!(rows[0].detail.contains("Chrome (browser driver)"));
    }

    #[test]
    fn each_group_header_renders_once() {
        let view = ExtensionsView::from_snapshot(snapshot(), ExtensionsTab::Plugins);
        let text = render_text(&view, 100, 24);

        assert_eq!(text.matches("Recommended").count(), 1, "{text}");
