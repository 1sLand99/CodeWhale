...
fn math_style() -> Style {
    Style::default().fg(Color::Rgb(80, 180, 230)).add_modifier(Modifier::ITALIC)
}
fn math_display_style() -> Style {
    Style::default().fg(Color::Rgb(60, 160, 220)).add_modifier(Modifier::BOLD)
}
/// Private: find the start of math delimiter ($, $$, \(, \[) in text.
fn find_math_start(text: &str) -> Option<usize> {
    let b = text.as_bytes();
    for (idx, &byte) in b.iter().enumerate() {
        if byte == b'$' { return Some(idx); }
        if byte == b'\\' && idx + 1 < b.len() {
            if b[idx + 1] == b'(' || b[idx + 1] == b'[' { return Some(idx); }
        }
    }
    None
}
...
fn find_math_end(text: &str) -> Option<(usize, bool)> {
    ...
    if b.starts_with(b"$$") {
        for i in 2..b.len().saturating_sub(1) {
            if b[i] == b'{' { continue; }
            if b[i..].starts_with(b"$$") { return Some((i, true)); }
        }
    } else if b.starts_with(b"$") && !b.starts_with(b"$$") {
        for i in 1..b.len() {
            if b[i] == b'{' { continue; }
            if b[i] == b'$' { return Some((i, false)); }
        }
        ...
        for i in 2..b.len().saturating_sub(1) {
            if b[i] == b'{' { continue; }
            if b[i..].starts_with(b"\\)") { return Some((i, false)); }
        }
    }
    None
}
fn math_delim_offset(text: &str) -> usize {
    let b = text.as_bytes();
    if b.starts_with(b"$$") || b.starts_with(b"\\[") || b.starts_with(b"\\(") { 2 }
    else if b.starts_with(b"$") { 1 }
    else { 1 }
}
...
pub fn render_latex_in_text(text: &str) -> String {
    ...
    while i < text.len() {
        ...
        if let Some((end, _is_display)) = find_math_end(remaining) {
            ...
            let close_len: usize = if remaining.as_bytes().get(end..end + 2) == Some(b"\\]") { 2 }
                else if remaining.as_bytes().get(end..end + 2) == Some(b"$$") { 2 }
                else if remaining.as_bytes().get(end..end + 1) == Some(b"$") { 1 }
                else if remaining.as_bytes().get(end..end + 2) == Some(b"\\)") { 2 }
                else { 0 };
            i += end + close_len;
        } else {
        ...
        match ch {
            '\\' => {
                ...
                while let Some(&c) = chars.peek() {
                    if c.is_ascii_alphabetic() { cmd.push(c); chars.next(); } else { break; }
                }
                if cmd.is_empty() {
                    if let Some(&c) = chars.peek() {
                        match c { '{' | '}' | '$' | '%' | '#' | '&' | '_' | ' ' => { chars.next(); }
                            _ => { out.push(c); chars.next(); }
                        }
                    }
                    continue;
                }
                match cmd.as_str() {
                    "frac" => { let n = render_latex_to_string(&read_braced(&mut chars));
                        let d = render_latex_to_string(&read_braced(&mut chars));
                        out.push_str(&format!("({}/{})", n, d)); }
                    "sqrt" => { let _n = read_optional_sqrt_root(&mut chars);
                        let r = render_latex_to_string(&read_braced(&mut chars));
                        out.push_str(&format!("\u{221a}({})", r)); }
                    "sum" => { out.push('\u{2211}'); skip_limits(&mut chars); }
                    "prod" => { out.push('\u{220f}'); skip_limits(&mut chars); }
                    "int" => { out.push('\u{222b}'); skip_limits(&mut chars); }
                    "iint" => { out.push_str("\u{222c}"); }
                    "iiint" => { out.push_str("\u{222d}"); }
                    "oint" => { out.push('\u{222e}'); }
                    "lim" => { out.push_str("lim"); skip_limits(&mut chars); }
                    "sin"|"cos"|"tan"|"cot"|"sec"|"csc"|"log"|"ln"|"lg"|"exp"
                        |"det"|"dim"|"ker"|"hom"|"max"|"min"|"sup"|"inf"|"arg"|"deg"|"mod"|"gcd"|"lcm"
                        => out.push_str(&cmd),
            ...
            '_' => { let sub = read_optional_arg(&mut chars);
...
fn read_braced(chars: &mut std::iter::Peekable<std::str::Chars>) -> String {
    ...
    let mut depth: u32 = 0;
    if chars.next_if_eq(&'{').is_some() { depth = 1; }
    while let Some(&c) = chars.peek() {
        match c {
            '{' => { depth += 1; s.push(c); chars.next(); }
            '}' => { depth = depth.saturating_sub(1); chars.next(); if depth == 0 { break; } s.push('}'); }
            _ => { s.push(c); chars.next(); }
        }
    }
    s
}
fn read_optional_arg(chars: &mut std::iter::Peekable<std::str::Chars>) -> String {
    if chars.peek() == Some(&'{') { read_braced(chars) }
    else { let mut s = String::new(); while let Some(&c) = chars.peek() {
        if c.is_alphanumeric() || c == '+' || c == '-' { s.push(c); chars.next(); } else { break; } } s }
}
fn read_optional_sqrt_root(chars: &mut std::iter::Peekable<std::str::Chars>) -> String {
    if chars.peek() == Some(&'[') { chars.next();
        let mut s = String::new(); while let Some(&c) = chars.peek() { if c == ']' { chars.next(); break; } s.push(c); chars.next(); } s }
    else { String::new() }
}
fn skip_limits(chars: &mut std::iter::Peekable<std::str::Chars>) {
    if chars.peek() == Some(&'_') { chars.next(); let _ = read_optional_arg(chars); }
    if chars.peek() == Some(&'^') { chars.next(); let _ = read_optional_arg(chars); }
}
...
fn append_subscript(s: &str, out: &mut String) {
    for c in s.chars() {
        out.push(match c {
            ...
            '=' => '\u{208c}', 'a' => '\u{2090}', 'e' => '\u{2091}', 'h' => '\u{2095}',
            'i' => '\u{1d62}', 'k' => '\u{2096}', 'l' => '\u{2097}', 'm' => '\u{2098}',
...
fn build_symbols() -> SymbolMap {
    ...
    for (k,v) in [("alpha","\u{03b1}"),("beta","\u{03b2}"),("gamma","\u{03b3}"),("delta","\u{03b4}"),
        ...
        ("varepsilon","\u{03b5}"),("vartheta","\u{03d1}"),("varphi","\u{03c6}")]
    { m.insert(k,v); }
    for (k,v) in [("Gamma","\u{0393}"),("Delta","\u{0394}"),("Theta","\u{0398}"),
        ("Lambda","\u{039b}"),("Xi","\u{039e}"),("Pi","\u{03a0}"),("Sigma","\u{03a3}"),
        ("Upsilon","\u{03a5}"),("Phi","\u{03a6}"),("Psi","\u{03a8}"),("Omega","\u{03a9}")]
    { m.insert(k,v); }
    for (k,v) in [("infty","\u{221e}"),("partial","\u{2202}"),("nabla","\u{2207}"),
        ("ell","\u{2113}"),("hbar","\u{210f}"),("Im","\u{2111}"),("Re","\u{211c}"),
        ("emptyset","\u{2205}"),("aleph","\u{2135}"),("angle","\u{2220}"),("perp","\u{22a5}"),
        ("parallel","\u{2225}"),("prime","\u{2032}"),("surd","\u{221a}"),("top","\u{22a4}")]
    { m.insert(k,v); }
    for (k,v) in [("in","\u{2208}"),("notin","\u{2209}"),("ni","\u{220b}"),("subset","\u{2282}"),
        ("supset","\u{2283}"),("subseteq","\u{2286}"),("supseteq","\u{2287}"),
        ("cup","\u{222a}"),("cap","\u{2229}"),("vee","\u{2228}"),("wedge","\u{2227}"),
        ("oplus","\u{2295}"),("otimes","\u{2297}"),("odot","\u{2299}"),
        ("forall","\u{2200}"),("exists","\u{2203}"),("neg","\u{00ac}"),
        ("sim","\u{223c}"),("simeq","\u{2243}"),("cong","\u{2245}"),("approx","\u{2248}"),
        ("neq","\u{2260}"),("ne","\u{2260}"),("equiv","\u{2261}"),
        ("le","\u{2264}"),("ge","\u{2265}"),("leq","\u{2264}"),("geq","\u{2265}"),
        ("ll","\u{226a}"),("gg","\u{226b}"),
        ("times","\u{00d7}"),("div","\u{00f7}"),("pm","\u{00b1}"),("mp","\u{2213}"),
        ("cdot","\u{00b7}"),("ast","\u{2217}"),("circ","\u{2218}"),("bullet","\u{2022}"),
        ("cdots","\u{2026}"),("ldots","\u{2026}"),("vdots","\u{22ee}"),("ddots","\u{22f1}")]
    { m.insert(k,v); }
    for (k,v) in [("mathbb{R}","\u{211d}"),("mathbb{C}","\u{2102}"),("mathbb{N}","\u{2115}"),
        ("mathbb{Q}","\u{211a}"),("mathbb{Z}","\u{2124}"),("mathbb{P}","\u{2119}"),("mathbb{H}","\u{210d}"),
        ("mathbb{F}","\u{1d53b}")]
    { m.insert(k,v); }
    for (k,v) in [("mathcal{L}","\u{2112}"),("mathcal{H}","\u{210b}"),("mathcal{R}","\u{211b}")]
    { m.insert(k,v); }
    m
}
...
mod tests {
    use super::*;
    #[test] fn test_superscript() { assert_eq!(render_latex_to_string("x^2"), "x\u{00b2}"); }
    #[test] fn test_subscript() { assert_eq!(render_latex_to_string("x_1"), "x\u{2081}"); }
    #[test] fn test_blackboard() { assert_eq!(render_latex_to_string(r"\mathbb{R}"), "\u{211d}"); }
    #[test] fn test_infty() { assert_eq!(render_latex_to_string(r"\infty"), "\u{221e}"); }
    #[test] fn test_inline_dollar() {
        let r = render_latex_in_text(r"text $x^2$ more");
        assert_eq!(r, "text x\u{00b2} more");
    }
    #[test] fn test_display_bracket() {
        let r = render_latex_in_text(r"text \[x^2\] more");
        assert_eq!(r, "text x\u{00b2} more");
    }
}
