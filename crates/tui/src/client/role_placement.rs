...
pub(crate) fn role_placement(role: &Role, dialect: WireDialect) -> RolePlacement {
    match (role, dialect) {
        ...
        // Chat Completions is the only dialect with a real in-transcript
        // system channel.
        (Role::System, WireDialect::ChatCompletions) => RolePlacement::System,
        // Responses has dropped in-transcript system messages since it was
        // written. Preserved: rejecting here would fail live sessions whose
        // compaction summaries are already on this path.
        (Role::System, WireDialect::OpenAiResponses) => RolePlacement::Omitted,
        // CHANGED: Anthropic used to forward `"role":"system"` verbatim and
        // take a provider 400 for it. It now fails locally, with the role and
        // the dialect named. Dropping it instead would silently delete a
        // compaction summary — the whole of a compacted history — so this
        // cell is a rejection, not an omission.
        (Role::System, WireDialect::AnthropicMessages) => RolePlacement::Rejected,
...
mod tests {
    use super::{
        RolePlacement, UnsupportedRoleForDialect, WireDialect, reject_unsupported_roles,
        role_placement,
    };
    ...
    #[test]
    fn system_placement_is_per_dialect_and_never_silently_lost_on_anthropic() {
        assert_eq!(
        ...
        // A dropped system message is a dropped compaction summary. Anthropic
        // cannot carry one, so the request is refused rather than quietly
        // shortened.
        assert_eq!(
    ...
    fn unknown_roles_never_reach_a_wire() {
        let role = Role::Unrecognized("developer".to_string());
        for dialect in DIALECTS {
            assert_ne!(
    ...
    #[test]
    fn seam_rejects_system_on_anthropic_and_names_the_message() {
        let messages = vec![
            message(Role::User),
            message(Role::Assistant),
            message(Role::System),
        ];
        let err = reject_unsupported_roles(&messages, WireDialect::AnthropicMessages)
            .expect_err("anthropic cannot carry an in-transcript system message");
        assert_eq!(
            err,
            UnsupportedRoleForDialect {
                index: 2,
    ...
    fn seam_accepts_what_each_dialect_can_carry() {
        ...
        // Omitted is not rejected: the OpenAI-shaped dialects have always
        // dropped these rather than refusing the turn.
        assert!(
...
mod adapter_agreement_tests {
    ...
    fn transcript() -> Vec<Message> {
        vec![
            ...
            message(Role::Unrecognized("developer".to_string()), "from later"),
        ]
    }
    ...
    fn chat_completions_maps_every_role_the_table_says_it_can_carry() {
        ...
        assert_eq!(
            roles(&items),
            vec!["user", "assistant", "assistant", "system"],
            "one wire entry per carried message, in transcript order: the \
             interrupted turn joins the assistant channel and the unknown \
             role is dropped",
        );
    ...
    #[test]
    fn responses_drops_system_and_unknown_and_marks_interrupted_history() {
        ...
        assert_eq!(
            ...
            "Responses has no in-transcript system channel and drops unknown roles",
        );
        let rendered = serde_json::to_string(&items).expect("serialize");
    ...
    /// Anthropic used to forward `message.role` verbatim, so `system` and any
    /// unknown role went out on the wire for the provider to reject. It now
    /// refuses to build such a message at all — and the seam refuses the
    /// request before this function is reached.
    #[test]
    fn anthropic_never_emits_a_role_outside_user_and_assistant() {
        for role in [
            Role::System,
            Role::Unrecognized("developer".to_string()),
