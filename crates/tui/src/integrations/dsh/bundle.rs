...
pub(crate) fn render_bundle_files(
    ...
    if skin {
        ...
        files.push((BUNDLE_CLIENT_FILE, skin::bundle_client_js()));
    }
    files
}
...
pub(crate) fn write_bundle(
    ...
    for (name, text) in render_bundle_files(codewhale_version, overlay_text, skin) {
...
/// On-disk client half vs the receipt's skin decision. `None` means in sync.
pub(crate) fn client_half_stale(bundle_dir: &Path, skin: bool) -> Option<String> {
    ...
    if skin {
        ...
        match std::fs::read_to_string(&client) {
            Ok(text) if text == skin::bundle_client_js() => None,
            Ok(_) => Some(
