...
async fn thread_handler(State(state): State<AppState>, Json(req): Json<ThreadRequest>) -> Response {
    ...
    {
        return match run_http_thread_message(&state, thread_id, input, max_output_tokens).await {
...
mod tests {
    ...
    async fn http_thread_message_runs_the_turn_instead_of_queueing_it() {
        ...
        let response =
            run_http_thread_message(&state, "thr_http".to_string(), "go".to_string(), None)
                .await
    ...
    async fn http_thread_message_without_a_runtime_is_a_typed_error() {
        ...
        let err = run_http_thread_message(&state, "thr_http".to_string(), "go".to_string(), None)
            .await
