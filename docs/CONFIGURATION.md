...
- `api_key` (string, required for hosted providers): must be non-empty for DeepSeek/hosted providers (or set the provider API key env var). Self-hosted SGLang, vLLM, and local Ollama can omit it. Direct Ollama Cloud at exact `https://ollama.com/v1` requires a saved `ollama` provider key or `OLLAMA_API_KEY`; other remote Ollama endpoints are custom routes and never inherit that official credential.
...
- `ollama` Cloud route: set `[providers.ollama] base_url = "https://ollama.com/v1"`, select an Ollama-hosted model id, and save a key from [Ollama account settings](https://ollama.com/settings/keys) with `codewhale auth set --provider ollama` or `OLLAMA_API_KEY`. A trailing slash and ASCII scheme/host casing are normalized; neighboring paths, HTTP downgrades, and lookalike hosts remain custom endpoints.
